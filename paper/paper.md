...
![NeuroCluster validation in synthetic data. A. Time-frequency representation (TFR) showing power differences between high (>0.50) and low (<0.50) expected value trials in synthetic data (1 channel, 100 trials, time-locked to "choice"). B. A significant positive cluster identified in the expected time-frequency region, consistent with the predefined association embedded in the synthetic dataset.](../workflow/Figure3.png){#fig:Fig3}
