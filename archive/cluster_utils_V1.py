...
'''
Cluster Utils V1 contains depreciated functions from old NeuroCluster implementations. Functions as dependency for NeuroCluster_V1_depreciated.ipynb in archive.
'''


def compute_tcritical(predictor_dims,tails=2, alternative = 'two-sided',alpha=0.05):
    """
    ...
    df = float(predictor_dims[0]-predictor_dims[1]-1)

    # Initialize array to store critical t-values based on tails
    tcritical = np.zeros(tails)
    
    if tails == 2:
        # Calculate critical t-value
        tcritical = t.ppf(1-(alpha/tails),df)
    elif (tails == 1) & (alternative == 'greater'):
        # Calculate critical t-value
        tcritical = t.ppf(1-alpha,df)
    elif (tails == 1) & (alternative == 'less'):
        # Calculate critical t-value
        tcritical = t.ppf(1-alpha,df) * -1

    return tcritical

def pixel_regression(time_data,regressor_data):

    """
    Run pixel-wise OLS regression model to extraxct beta coefficient and t-statistic. 

    Args:
    - time_data (numpy array): Target variable.
    - regressor_data (numpy array): Feature matrix. 
    ...
    y = time_data.copy()
    X = regressor_data.copy() 
    X = sm.add_constant(X) # must add constant for statsmodels OLS
    pixel_model = sm.OLS(y, X).fit() # fit regression model
    beta = pixel_model.params[1:] # extract beta coefficient(s) excluding intercept 
    tstat = pixel_model.tvalues[1:] # extract t statistic(s) excluding intercept 

    return beta, tstat 

##### commented out masking - broke my code 
def tfr_cluster_test(elec_data, predictor_data, tcritical, zscore=False, clust_def=None,output='all'):
    ...
    num_freq = elec_data.shape[1] # num frequencies 
    num_time = elec_data.shape[2] # num time points 
    ...
    for f in range(num_freq): 
        ...
        for t in range(num_time):
            time_data = np.squeeze(freq_data[:,t]) # matrix is num_epochs x 1 
